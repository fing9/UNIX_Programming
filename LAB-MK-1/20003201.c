...
void ls_m(char **res){
    char cwd[512], *buf, fd, m[2] = ".\0", f[3] = "..\0";
    int ret;
	DIR* dir = NULL;
    ...
    while((dirEntry = readdir(dir)) != NULL) {
        if(strcmp(dirEntry->d_name, m) && strcmp(dirEntry->d_name, f)) {
			...
			if(ret != -1 && !S_ISREG(s.st_mode))
				...
				printf("F ");
            if(ret != -1) {
                buf = ctime(&s.st_mtime);
                buf[strlen(buf) - 1] = '\0';
                printf("0%o  %d  %d  %d  %-7lld %-27s %s\n", s.st_mode & 0777, s.st_nlink, s.st_uid, s.st_gid, s.st_size, buf, dirEntry->d_name);
            }
        }
    }
}
